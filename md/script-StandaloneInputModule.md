...
The module sends move events and submit / cancel events in response to Input tracked via the [Input](class-InputManager) window. This works for both keyboard and controller input. The tracked axis and keys can be configured in the module's inspector.
...
- Send a Move event to the selected object if a valid axis from the [Input](class-InputManager) window is entered
